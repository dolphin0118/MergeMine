...
public class Cup : MonoBehaviour {
    ...
    void Drop() {
        if(Input.GetKey(KeyCode.Space)) {
            transform.Rotate(new Vector3(1, 0, 0) * Time.deltaTime * Rotate_degree);
        }
    }
    ...
    {
        Ray ray = Camera.main.ScreenPointToRay(Input.mousePosition);
        if (Physics.Raycast(ray, out hitFloor, Mathf.Infinity, LayerMask.GetMask("Cup")))
        {
